...
@Controller('transbank') // Prefijo de las rutas
export class TransbankController {
  ...
  @Post('iniciar')
  async iniciarPago(@Body() body: { amount: number; sessionId: string; buyOrder: string; returnUrl: string }) {
    try {
  ...
  async confirmarPago(@Body() body: { token: string }) {
    try {
      ...
      const response = await this.transbankService.confirmarTransaccion(body.token);
